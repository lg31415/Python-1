...
def loaddata():
	...
	y=np.loadtxt('../data/labels', dtype=np.float32,delimiter=',')  #converters={ 0 : lambda ch : ord(ch)-ord('A')})
	#随机抽取生成训练集和测试集，其中训练集的比例为60%，测试集40%
	train_samples,test_samples,train_labels,test_labels = cross_validation.train_test_split(X, y, test_size=0.4, random_state=0)
	return	train_samples,test_samples,train_labels,test_labels

'''
	训练和测试
'''
def cnn_tensor(train_samples,test_samples,train_labels,test_labels):
	...
	#模型保存（ok）
	# 方法1：只保存图模型
	tf.train.export_meta_graph(filename='export_meta_model.meta') #模型导出
	...
	saver.save(sess,"saver_checkpoint")
	...
	# 生成混淆矩阵(对误判情况进行分析：行代表实际，列代表预测)(//TODO)
	'''
	pre_labels=tf.nn.top_k(tf.arg_max(y_conv, 1),feed_dict={x: test_samples},1)
	print pre_labels
	pre_char_labels=[ chr(l.index(1)+65) for l in pre_labels ]
	test_char_labels=[ chr(l.index(1)+65) for l in test_labels ]
...
if __name__ == "__main__":
	train_samples,test_samples,train_labels,test_labels=loaddata()
	#cnn_tensor(train_samples,test_samples,train_labels,test_labels)
	testmodel1()
