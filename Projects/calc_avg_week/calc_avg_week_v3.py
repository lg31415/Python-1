...
def calc_avg_week(conn,cur,stadate,tblname):
    ...
    sql="use pgv_stat;select %s as 统计日期,a.source as 安装渠道,\
         ...
         a.date>'%s' and a.date<='%s' group by a.source order by a.source;" %(stadate,tblname,startdate1,enddate1)
    print '[sql]\n',sql
    cur.execute(sql)
    ...
    sql="use pgv_stat;select %s as 统计日期,a.source as 安装渠道,\
         ...
         from %s as a inner join xmp_install_source_total b  on b.source=a.source where a.date=b.date and\
         a.date>'%s' and a.date<='%s' group by a.source order by a.source;" %(stadate,tblname,startdate3,enddate3)
    print '[sql]\n',sql
    cur.execute(sql)
    results=cur.feachall()
    for item in results:
    ...
    sql="use pgv_stat;select %s as 统计日期,a.source as 安装渠道,\
         ...
         from %s as a inner join xmp_install_source_total b  on b.source=a.source where a.date=b.date and\
         a.date>'%s' and a.date<='%s' group by a.source order by a.source;" %(stadate,tblname,startdate7,enddate7)
    print '[sql]\n',sql
    cur.execute(sql)
    results=cur.feachall()
    for item in results:
    ...
    sql="use pgv_stat;select %s as 统计日期,a.source as 安装渠道,\
         ...
         from %s as a inner join xmp_install_source_total b  on b.source=a.source where a.date=b.date and\
         a.date>'%s' and a.date<='%s' group by a.source order by a.source;" %(stadate,tblname,startdate14,enddate14)
    print '[sql]\n',sql
    cur.execute(sql)
    results=cur.feachall()
    for item in results:
    ...
    sql="use pgv_stat;select %s as 统计日期,a.source as 安装渠道,\
         ...
         from %s as a inner join xmp_install_source_total b  on b.source=a.source where a.date=b.date and\
         a.date>'%s' and a.date<='%s' group by a.source order by a.source;" %(stadate,tblname,startdate28,enddate28)
    print '[sql]\n',sql
    cur.execute(sql)
    results=cur.feachall()
    for item in results:
    ...
    #将查询结果输出到文件中
    sql='''set names utf8;use pgv_stat;select mdate as 日期，source as 安装渠道，1dayavg as 1日留存均值，3dayavg as 3日留存均值，7dayavg as 7日留存均值 \
         14dayavg as 14日留存均值，28dayavg as 28日留存均值，7dayavg as 周留存均值 from xmp_week_avg'''
