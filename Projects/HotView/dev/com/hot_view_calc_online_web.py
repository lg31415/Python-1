...
############# 全局变量
top_cid_filesize='toplist'						# 待添加高潮模式的影片信息文件列表(取自数据库)
datapath='/usr/local/sandai/xmp_hotview/data'   # 原始采集数据文件位置(getview_from_local)
hotrespath='D://dad'                            # 高潮模式结果信息存放位置
exceptmovie='./exceptmovie'                     # 高潮处理异常
...
'''
# 预处理: 获取cid和filesize信息(获取toplist)
...
'''
def getview_from_local(datapath='/usr/local/sandai/xmp_hotview/data',updateTop=True):
    if updateTop or not os.path.exists(top_cid_filesize):
        update_top_cidfilesize(top_cid_filesize)
...
def savehigh(cid,filesize,highdur,hotrespath='/data1/xmp_hotview/highdur'):
    fisrtpath=cid[0:2]
    secondpath=cid[2:4]
