...
def readData(conn,cur,stadate):
    xmp_day_play_file="%s/xmp_day_play_%s" %(file_path,stadate)
    #数据准备(hive数据到本地文件)
...
def mergeResult(conn,cur,stadate):
    ...
    for index,item in enumerate(res):
        ...
        results.append([movieid,mtype,title,uv,pv])


    #将 date | movieid |type | title | uv | pv  数据先插入到中间结果表xmp_day_play_mid中
    if len(results)!=0:
        sql='delete from pgv_stat.xmp_day_play where date="%s"' % (stadate)
        cur.execute(sql)
        for iteml in results:
            sql="insert into pgv_stat.xmp_day_play values('%s','%s','%s','%s','%s','%s')" %(stadate,iteml[0],iteml[1],iteml[2],iteml[3],iteml[4])  #problem?
