...
'''
  功能注释：播放排行统计
  对比3.4版本的改进之处，是对输入参数的处理，指定开始和截止日期，统计日期间隔
'''
...
def initTable():
    ...
    '''
    sql='create table if not exists xmp_day_play_mid(mdate date,movieid varchar(10),mtype varchar(10),mtitle varchar(30),uv int(10),pv int(10)) character set utf8'
    cur.execute(sql)
    conn.commit()
...
def mergeResult(conn,cur,stadate):
    ...
    for index,item in enumerate(res):
        movieid=item[0]
        title=item[1].decode('utf8') #.encode('utf8') #title可能为空
        if title=="":
            title='Unknown'
        uv=item[2]
        pv=item[3]
    ...
    if len(results)!=0:
        for iteml in results:
            sql="insert into xmp_day_play_mid values('%s','%s','%s','%s','%s','%s')" %(stadate,iteml[0],iteml[4],iteml[1],iteml[2],iteml[3])  #problem?
            print(sql)
            cur.execute(sql.encode('utf8'))
            conn.commit()
    else:
...
def closePro(conn,cur):
    ...
    cur.execute('drop table xmp_day_play_mid')
    conn.commit()
...
if __name__ == "__main__":
    ...
    #利用时间日期计算函数
    startdate_struct=time.striptime(sys[1],'%Y%m%d') #time.struct_time
    ...
    enddate_struct=time.strptime(sys[2],'%Y%m%d')
    ...
    while startdate_string<enddate_string:
        ...
        startdate_date=startdate_string+timedelta(days=1)
        startdate_string=time.strftime('%Y%m%d',startdate_date.timetuple())
