...
def calc_avg_week(conn,cur,stadate,tblname):
    ...
    cmd='mysql --defaults-file=/etc/my5.cnf  -uroot -psd-9898w -e "%s">%s' %(sql,tblname+'.txt')
    #print '[cmd]\n',cmd
    os.system(cmd)
...
if __name__ == "__main__":
    ...
    ## 建立连接并初始化中间表
    conn=MySQLdb.connect(host='127.0.0.1',port=3307,user='root',passwd='sd-9898w') #新建的数据库
    if not conn.open:
    ...
    conn.commit()
    #先清空中间表数据
    sql="delete from pgv_stat_mid.xmp_week_avg;" 
    cur.execute(sql)
    conn.commit()
