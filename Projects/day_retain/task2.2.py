...
'''
  功能注释：日留存计算，分按渠道和按版本
  注意事项：
  1）尽可能的在Linux上进行开发，和在上面解决问题
  2）语句尽量要简单，单条语句执行的功能不要太复杂，避免使用子查询
  3）注意代码的书写风格
'''
...
merge_sql='''create table if not exists mid_table
             select *,count(*) as num from temp_merge_table group by peerid
...
#按渠道计算留存率(更新表)
sel_sql='update result_source_remain_table set clc_remain=clc_num/clc_totalnum'
cur.execute(sel_sql)
conn.commit()
print('-------source_ratio already---------')

#按版本计算留存率（更新表）
sel_sql='update result_version_remain_table set clc_remain=clc_num/clc_totalnum'
cur.execute(sel_sql)
conn.commit()
