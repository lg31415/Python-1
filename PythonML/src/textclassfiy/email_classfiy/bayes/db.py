...
'''
create table word(word, doctype, count, doc_count);
...
class Db:
	...
	def update_word_count(self, c, doctype, word, num_to_add_to_count):
		c.execute('select count,doc_count from word where doctype=? and word=?', (doctype, word))
		r = c.fetchone()
		if r:
			c.execute('update word set count=?, doc_count=? where doctype=? and word=?', (r[0] + num_to_add_to_count, r[1] + 1, doctype, word))
		else:
			c.execute('insert into word (doctype, word, count, doc_count) values (?,?,?,?)', (doctype, word, num_to_add_to_count, 1))

		#print 'word, count: ', word, num_to_add_to_count
	...
	def get_word_doc_count(self, doctype, word):
		...
		c = self.conn.cursor()
		try:
			c.execute('select doc_count from word where doctype=? and word=?', (doctype, word))
			r = c.fetchone()
			if r:
				return r[0]
			else:
