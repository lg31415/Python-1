...
#发送txt格式的邮件
def send_txtmail(to_list,sub,content):
...
#发送html格式的邮件
def send_htmlmail(to_list,sub,content):                         #to_list：收件人；sub：主题；content：邮件内容
...
if __name__ == '__main__':
    ...
    if send_htmlmail(mailto_list,"关于邮件的测试","<a href='http://www.baidu.com'>这是重要的内容</a>"):
        ...
        print "发送html失败"
    '''
    if send_htmlmail(mailto_list,"以txt发送邮件内容的测试","这是txt邮件内容"):
